...
<div class="space-y-6">
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                <div>
                    <h3 class="text-lg font-medium text-gray-800 dark:text-white">Subvarian Produk</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambah subvarian seperti Small/Medium/Large.</p>
                </div>
                <button
                    type="button"
                    wire:click="addSubvariant"
                    class="shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600"
                >
                    Tambah Subvarian
                </button>
            ...
            <div class="p-4 sm:p-6 space-y-4">
                <x-common.input-error for="subvariants" class="text-xs text-error-600" />

                <div class="hidden sm:block overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-900">
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Nama Subvarian</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Harga</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Diskon %</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Harga Setelah Diskon</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">HPP</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            @foreach ($subvariants as $subIndex => $subvariant)
                                @php
                                    $subKey = (string) ($subvariant['key'] ?? $subIndex);
                                    $subPrice = (float) ($subvariant['price'] ?? 0);
                                    $subPercent = $subvariant['percent'] === null || $subvariant['percent'] === '' ? null : (int) $subvariant['percent'];
                                    $subPriceAfterDiscount = $subPercent === null ? null : round($subPrice - ($subPrice * ($subPercent / 100)), 2);
                                    $subHpp = (float) (($hppBySubvariantKey[$subKey] ?? 0));
                                    $subSellingPrice = $subPriceAfterDiscount === null ? $subPrice : (float) $subPriceAfterDiscount;
                                    $subMargin = $subSellingPrice - $subHpp;
                                @endphp
                                <tr wire:key="subvarian-row-{{ $subKey }}" class="bg-white dark:bg-gray-950/30">
                                    <td class="px-4 py-3 align-top">
                                        <input
                                            wire:model.live="subvariants.{{ $subIndex }}.name"
                                            type="text"
                                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                                            placeholder="Contoh: Small"
                                        />
                                        <x-common.input-error :for="'subvariants.'.$subIndex.'.name'" />
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <input
                                            x-data="currencyInput($wire.entangle('subvariants.{{ $subIndex }}.price'))"
                                            wire:model.live="subvariants.{{ $subIndex }}.price"
                                            x-model="displayValue"
                                            @input="handleInput"
                                            type="text"
                                            inputmode="numeric"
                                            ...
                                            placeholder="0"
                                        />
                                        <x-common.input-error :for="'subvariants.'.$subIndex.'.price'" />
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <input
                                            wire:model.live="subvariants.{{ $subIndex }}.percent"
                                            type="number"
                                            min="0"
                                            max="100"
                                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                            placeholder="-"
                                        />
                                        <x-common.input-error :for="'subvariants.'.$subIndex.'.percent'" />
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                                            {{ $subPriceAfterDiscount === null ? '-' : number_format($subPriceAfterDiscount, 2, ',', '.') }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                                            Rp{{ number_format($subHpp, 0, ',', '.') }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 align-top">
                                        <div class="h-10 w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm {{ $subMargin < 0 ? 'text-error-600 dark:text-error-500' : 'text-gray-700 dark:text-gray-300' }} dark:border-gray-800 dark:bg-gray-900">
                                            Rp{{ number_format($subMargin, 0, ',', '.') }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 align-top text-right">
                                        <button
                                            type="button"
                                            wire:click="removeSubvariant('{{ $subKey }}')"
                                            class="shadow-theme-xs inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]"
                                        >
                                            Hapus
                                        </button>
                ...
                <div class="sm:hidden space-y-3">
                    @foreach ($subvariants as $subIndex => $subvariant)
                        @php
                            ...
                            $subMargin = $subSellingPrice - $subHpp;
                        @endphp
                        <div wire:key="subvarian-card-{{ $subKey }}" class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
                            <div class="flex items-center justify-between">
                                <p class="text-sm font-semibold text-gray-800 dark:text-white/90">Subvarian</p>
                                <button type="button" wire:click="removeSubvariant('{{ $subKey }}')" class="text-sm font-semibold text-error-700 hover:text-error-800 dark:text-error-400 dark:hover:text-error-300">
                                    Hapus
                                </button>
                            </div>
                            ...
                            <div class="mt-3 space-y-3">
                                <div>
                                    <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Nama Subvarian</label>
                                    <input
                                        wire:model.live="subvariants.{{ $subIndex }}.name"
                                        type="text"
                                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="shadow-theme-xs rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-gray-800 dark:bg-gray-950/30">
                                        <p class="text-xs text-gray-500 dark:text-gray-400">HPP</p>
                                        <p class="font-semibold text-gray-800 dark:text-white/90">Rp{{ number_format($subHpp, 0, ',', '.') }}</p>
                                    </div>
                                    <div class="shadow-theme-xs rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:border-gray-800 dark:bg-gray-950/30">
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Margin</p>
                                        <p class="font-semibold {{ $subMargin < 0 ? 'text-error-700 dark:text-error-400' : 'text-gray-800 dark:text-white/90' }}">Rp{{ number_format($subMargin, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
