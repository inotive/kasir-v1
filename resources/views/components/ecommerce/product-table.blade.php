...
<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col justify-between gap-5 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center dark:border-gray-800">
        ...
        <div class="flex gap-3">
            @can('products.create')
                <a wire:navigate href="{{ route('products.create') }}" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition">
    ...
    <div class="custom-scrollbar overflow-x-auto">
        <table class="w-full table-auto">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($products as $product)
                    ...
                    <tr>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400 sm:hidden">{{ $product->category?->name ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            ...
                            @elseif (abs($priceMin - $priceMax) < 0.005)
                                <p class="text-sm font-medium text-gray-800 dark:text-white/90">Rp {{ number_format($priceMin, 0, ',', '.') }}</p>
                            @else
                                <p class="text-sm font-medium text-gray-800 dark:text-white/90">Rp {{ number_format($priceMin, 0, ',', '.') }} - {{ number_format($priceMax, 0, ',', '.') }}</p>
                            @endif

                            @if ($promoMin !== null && $promoMax !== null)
                                @if (abs($promoMin - $promoMax) < 0.005)
                                    <p class="mt-1 text-xs font-semibold text-success-700 dark:text-success-400">Promo Rp {{ number_format($promoMin, 0, ',', '.') }}</p>
                                @else
                                    <p class="mt-1 text-xs font-semibold text-success-700 dark:text-success-400">Promo Rp {{ number_format($promoMin, 0, ',', '.') }} - {{ number_format($promoMax, 0, ',', '.') }}</p>
                                @endif
                            @endif
                        </td>
                        <td class="px-5 py-4">
                            ...
                            @if ($hppMin === null || $hppMax === null)
                                <p class="text-sm text-gray-500 dark:text-gray-400">-</p>
                            @elseif (abs($hppMin - $hppMax) < 0.005)
                                <p class="text-sm font-medium text-gray-800 dark:text-white/90">Rp {{ number_format($hppMin, 0, ',', '.') }}</p>
                            @else
                                <p class="text-sm font-medium text-gray-800 dark:text-white/90">Rp {{ number_format($hppMin, 0, ',', '.') }} - {{ number_format($hppMax, 0, ',', '.') }}</p>
                            @endif
                            @if (($product->variants_with_recipes_count ?? 0) > 0 && ($hppMin ?? 0) <= 0.005 && ($hppMax ?? 0) <= 0.005)
                            ...
                            @endif
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ (int) ($product->variants_count ?? 0) }} varian</p>
                        </td>
                        ...
                        @canany(['products.edit', 'products.delete'])
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    @can('products.edit')
                                        <a wire:navigate href="{{ route('products.edit', $product) }}" class="shadow-theme-xs inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                                            Edit
                                        </a>
                                    @endcan
                                    @can('products.delete')
                                        <button type="button" onclick="confirm('Hapus produk ini?') || event.stopImmediatePropagation()" wire:click="deleteProduct({{ $product->id }})" class="shadow-theme-xs inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                                            Hapus
                                        </button>
                                    @endcan
                                </div>
                ...
                @empty
                    <tr>
                        <td colspan="@canany(['products.edit', 'products.delete']) 8 @else 7 @endcanany" class="px-5 py-10">
    ...
    <div class="border-t border-gray-200 px-5 py-4 dark:border-gray-800">
        {{ $products->links('livewire.pagination.admin') }}
    </div>
</div>
