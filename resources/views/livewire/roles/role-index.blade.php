    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-2">
            ...
            <div class="inline-flex items-center rounded-xl border border-gray-200 bg-white p-1 dark:border-gray-800 dark:bg-gray-900">
                <button type="button" wire:click="setTab('list')" @class([
                ...
                <button type="button" wire:click="setTab('guide')" @class([
                    'rounded-lg px-3 py-2 text-sm font-semibold transition',
                ...
                <button type="button" wire:click="setTab('permissions')" @class([
                    'rounded-lg px-3 py-2 text-sm font-semibold transition',
        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                @if ($grouped->isNotEmpty())
                    <div class="mt-4 flex flex-wrap gap-2">
                        @foreach ($grouped as $groupKey => $items)
                            <a href="#perm-group-{{ $groupKey }}" class="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-white/[0.03]">
            ...
            <div class="p-5 space-y-6">
                @forelse ($grouped as $groupKey => $items)
                    <div class="space-y-3">
                        <div id="perm-group-{{ $groupKey }}" class="flex items-center justify-between scroll-mt-28">
                            <div class="text-sm font-semibold text-gray-800 dark:text-white/90">
                                {{ \App\Helpers\RbacLabelHelper::permissionGroup((string) $groupKey) }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $items->count() }} hak akses</div>
                        </div>
                                ...
                                <details class="group rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/30">
                                    <summary class="flex cursor-pointer list-none items-start justify-between gap-3">
                                        ...
                                        <div class="flex shrink-0 flex-col items-end gap-1">
                                            @if ((bool) ($risk['sensitive_data'] ?? false))
                                            ...
                                            @endif
                                            @if ((bool) ($risk['financial_risk'] ?? false))
                                            ...
                                            @endif
                                            @if ((bool) ($risk['system_risk'] ?? false))
                                    ...
                                    <div class="mt-4 grid gap-4 lg:grid-cols-2">
                                        <div class="space-y-2">
                                            ...
                                            <div class="text-sm text-gray-700 dark:text-gray-300">{{ (string) ($guide['summary'] ?? '-') }}</div>
                                        </div>
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    @foreach ($related as $r)
                                                        <span class="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                                                            {{ \App\Helpers\RbacLabelHelper::permission((string) $r) }}
                                                        </span>
                                                    @endforeach
                                                </div>
                                            @else
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @forelse ($roles as $role)
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ \App\Helpers\RbacLabelHelper::role((string) $role->name) }}</td>
                                <td class="px-6 py-4">
